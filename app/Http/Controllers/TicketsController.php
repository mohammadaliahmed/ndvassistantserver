    ...
    public function ticketDetail($id)
    {
        ...
        if ($user->hasRole('admin')) {
            ...
            if ($user->hasRole('staff')) {
                $ticket = Tickets::where(['assigned_to' => Auth::id(), 'id' => $id])->first();

            } else {
                $ticket = Tickets::where(['user_id' => Auth::id(), 'id' => $id])->first();
...
//            $ticket = Tickets::where(['assigned_to'=> Auth::id(), 'id' => $id])->orWhere(['user_id'=> Auth::id(), 'id' => $id])->first();
    ...
    public function addReply(Request $request)
    {
        ...
        if (Auth::user()->hasRole('admin')) {
            ...
            $userToNotify = User::find($ticket->user_id);
            $sendNotification = new SendNotification();
            $sendNotification->sendPushNotification($userToNotify->fcmKey,
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $sendNotification->sendPushNotification($user->fcmKey,
            "Ticket status updated " . $tickets->subject,
